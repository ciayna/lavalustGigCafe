...
<form style="background: white; border-radius: 10px; padding: 20px 20px" action="/save" method="post">
		<div class="table-wrapper">
				...
				<div class="table-title">
                    ...
                    <div class="form-group">
						<label>Event</label>
                        <br>    
						<select id="event" name="event" style="width: 1070px; height: 40px; border-radius: 5px #dadfe3" class="form-group" required>
                        <?php if ($selected['event'] == 'Birthday'): ?>
                            <option value="Birthday" selected>Birthday</option>
                            <option value="Wedding">Wedding</option>
                            <option value="Anniversary">Anniversary</option>
                            <option value="Others...">Others...</option>
                        <?php elseif($selected['event'] == 'Wedding'): ?>
                            <option value="Birthday">Birthday</option>
                            <option value="Wedding" selected>Wedding</option>
                            <option value="Anniversary">Anniversary</option>
                            <option value="Others...">Others...</option>
                        <?php elseif($selected['event'] == 'Anniversary'): ?>
                            <option value="Birthday">Birthday</option>
                            <option value="Wedding">Wedding</option>
    ...
    <script>
        var now = new Date("<?php if(isset($selected['date'])): ?><?= $selected['date']; endif; ?>").toISOString()
        now=now.split('T')
